...
export default {
  ...
  created () {
    ...
    this.jurisdictionValue = this._authority.length ? this._authority[0].responsibilityId : null
    console.log(this._authority, 'jurisdictionButs', this.jurisdictionValue)
  },
  computed: {
    // 职责组
    jurisdictionOptions () {
      return this._authority.map((e) => ({ label: e.responsibilityName, value: e.responsibilityId, disabled: e.disabled }))
    },
    // 按钮组
    jurisdictionButs () {
  ...
  methods: {
    getData (fn) {
      return async (...arg) => {
        const integration = arg.filter((e) => isObject(e))
